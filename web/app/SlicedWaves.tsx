...
// Adaptado do SlicedWaves (React Bits) para TypeScript.
// Adição: escuta pointermove na janela (usando o retângulo do container),
// para a interação com o mouse funcionar com o fundo atrás do conteúdo clicável.
...
export default function SlicedWaves({
  ...
  useEffect(() => {
    ...
    const pointFromClient = (clientX: number, clientY: number) => {
      ...
      targetMouse = [(clientX - rect.left) / rect.width, 1.0 - (clientY - rect.top) / rect.height];
      targetActive = 1;
    };
    ...
    const loop = (t: number) => {
      ...
      um[0] = currentMouse[0];
      um[1] = currentMouse[1];
      (program.uniforms.uMouseActive.value as number) = currentActive;
      renderer.render({ scene: mesh });
  ...
  useEffect(() => {
    ...
    (u.uColumns.value as number) = Math.max(1, Math.round(columns));
    (u.uRows.value as number) = Math.max(1, Math.round(rows));
    (u.uThickness.value as number) = barThickness;
    (u.uSpeed.value as number) = speed;
    (u.uTravel.value as number) = travel;
    (u.uWaveSpread.value as number) = waveSpread;
    (u.uRowOffset.value as number) = rowOffset;
    (u.uSoftness.value as number) = softness;
    (u.uGlow.value as number) = glow;
    (u.uBrightness.value as number) = brightness;
    (u.uContrast.value as number) = contrast;
    (u.uOpacity.value as number) = opacity;
    (u.uVertical.value as number) = orientation === "vertical" ? 1.0 : 0.0;
    (u.uAlternate.value as number) = alternate ? 1.0 : 0.0;
    (u.uMouseStrength.value as number) = mouseStrength;
    (u.uMouseRadius.value as number) = mouseRadius;
    (u.uEnableMouse.value as number) = mouseInteraction ? 1.0 : 0.0;
    (u.uGrain.value as number) = grain ? 1.0 : 0.0;
    (u.uGrainIntensity.value as number) = grainIntensity;
    (u.uLightMode.value as number) = lightMode ? 1.0 : 0.0;
